...
export default class LevelOne extends Phaser.Scene {
  ...
  create() {
   ...
   Right.on('pointerup', () => {this.scene.start('LevelTwo');})

   var Left = this.add.image(277, 431, 'LeftA')
    Left.setInteractive()

  Left.on('pointerup', () => {this.scene.start('GameScene');})


  var Start = this.add.text(473, 580, 'START')
    Start.setInteractive()

  Start.on('pointerup', () => {this.scene.start('ArrozConLeche');})



    this.input.keyboard.on('keydown-SPACE', () => {this.scene.start('MenuScene'); });

    this.input.keyboard.on('keydown-ENTER', () => {this.scene.start('ArrozConLeche');});
  }
}
